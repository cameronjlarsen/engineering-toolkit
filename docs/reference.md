...
| Multi-model panels (arena, architect, interrogate) | Provider dispatch restores the upstream frontier quad: `claude-code/fable@max`, `codex/gpt-6-sol@max`, `grok/grok-4.6@xhigh`, `claude-code/opus@xhigh`. Same-provider lanes stay native; external lanes use the bundled runner. |
