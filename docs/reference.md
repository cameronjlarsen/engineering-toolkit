...
This checkout ships as a Claude Code marketplace containing one plugin (`et`). Add the checkout as a local marketplace, install `et@engineering-toolkit`, and reload plugins.
...
The same plugin carries a `.codex-plugin/plugin.json` manifest and a root `.agents/plugins/marketplace.json`. Add this checkout as a local Codex marketplace, then add `et`.
