...
Do not install Lauren's original pstack alongside it. Install `et` from the marketplace panel. Run `/setup-engineering-toolkit`. Setup writes grammar 3 into `~/.cursor/rules/engineering-toolkit-models.mdc`. Grammar 2 sheets still load. It does not write upstream Task slugs.
