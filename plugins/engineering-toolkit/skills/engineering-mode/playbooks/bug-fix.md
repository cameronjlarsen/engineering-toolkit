...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation through provider dispatch using your configured bug-fix descriptor (default `codex/gpt-6-sol@max`) with `isolated-write`, a dedicated worktree, and a specific scope.
