...
These skills share one tree across Claude Code, Codex, and Cursor. Read [`references/provider-dispatch.md`](references/provider-dispatch.md) whenever a configured role launches. It defines the typed Route values and `app/model@effort` wire values, native/external route table, launcher, isolation, receipts, dropout policy, and `beginSoloRole` / `observeSoloRole`. Only `capacity-exhausted` yields another `LanePlan`. Panel commas stay concurrent fan-out; each comma entry may itself be a `then` chain. Children never choose routes. When a skill names a Claude tool or built-in skill (`run`, `verify`, `plugin-dev:skill-development`), read [`references/codex-tools.md`](references/codex-tools.md) on Codex and [`references/cursor-tools.md`](references/cursor-tools.md) on Cursor.
...
- **Shipping.** The half after Babysit. Independently verifying a green stack, then landing the contiguous verified run bottom-up through the tagged forge from Opening a PR: Origin, Azure DevOps, or GitHub. `playbooks/shipping.md`.
