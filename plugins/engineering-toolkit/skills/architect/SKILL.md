...
**Dispatch contract.** Architect's Arena uses the centralized provider routing in [`provider-dispatch.md`](../engineering-mode/references/provider-dispatch.md). Configured entries are typed Routes written as `app/model@effort`, not native model slugs. On Codex, resolve remaining Claude tool names via [`codex-tools.md`](../engineering-mode/references/codex-tools.md).
