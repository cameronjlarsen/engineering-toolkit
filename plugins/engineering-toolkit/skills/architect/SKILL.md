...
**Dispatch contract.** Architect's Arena uses the centralized provider routing in [`provider-dispatch.md`](../engineering-mode/references/provider-dispatch.md). Configured entries are typed Routes written as `provider:model@effort`, not native model slugs. On Codex, resolve remaining Claude tool names via [`codex-tools.md`](../engineering-mode/references/codex-tools.md).
...
Take the runners from the `architect runners` line in the current harness's `engineering-toolkit-models` sheet, in place of the `arena runners` line. If the sheet or that line is missing, use `claude:fable@max`, `codex:gpt-6-sol@max`, `grok:grok-4.7@xhigh`, `claude:opus@xhigh`. Alias and dropout entries follow the runner rules in the **arena** skill's Phase A.
