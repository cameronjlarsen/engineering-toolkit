...
**Dispatch contract.** Architect's Arena uses the centralized provider routing in [`provider-dispatch.md`](../engineering-mode/references/provider-dispatch.md). Configured entries are typed Routes written as `app/model@effort`, not native model slugs. Open each runner with `beginSoloRole`. Capacity on one lane may yield that lane's next hop; it does not rewrite the panel. On Codex, resolve remaining Claude tool names via [`codex-tools.md`](../engineering-mode/references/codex-tools.md).
...
Use your configured architect runners (defaults `claude-code/fable@max`, `codex/gpt-5.6-sol@max`, `grok/grok-4.6@xhigh`, `claude-code/opus@xhigh`).
