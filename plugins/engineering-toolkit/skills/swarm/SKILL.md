...
4. Pick the worker descriptor from the `swarm workers` line in the current harness's `engineering-toolkit-models` sheet. If the sheet or that line is missing, use `grok/grok-4.7@xhigh`. For `auto` or `inherit-parent`, omit the Route so the workers run on the parent model. For a model race, name each arm's descriptor up front.
